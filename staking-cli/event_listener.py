    ...
    async def _connect_and_listen(self, filter_params: dict, subscription_type: str):
        ...
        async with AsyncWeb3(WebSocketProvider(self.ws_url)) as w3:
            ...
            try:
                ...
                # Start catch-up in background (non-blocking)
                # The deduplication system will handle any overlap between catch-up and real-time
                if self.last_block_number:
                    asyncio.create_task(self._catchup_missed_events(w3, filter_params))
                else:
...
async def main():
    ...
    db_config = None
    if args.db_host or args.db_user:
        # If any DB param is provided, build config
        config = {}
        try:
            config = read_config(args.config_path)
        except:
        ...
        db_config = {
            'host': args.db_host or config.get('database', {}).get('host', 'localhost'),
            'port': args.db_port,
            'database': args.db_name,
            'user': args.db_user or config.get('database', {}).get('user', 'postgres'),
